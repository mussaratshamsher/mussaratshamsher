<!-- ========================================================= -->

<!--                         HERO                              -->

<!-- ========================================================= -->
<!-- ===================== HERO ===================== -->

<table>
  <tr>
    <td width="35%" align="center">
      <img
        src="https://media.giphy.com/media/93UOscPyDH8cdRfSaT/giphy.gif"
        alt="Coding Animation"
        width="280"
      />
    </td>

    <td width="65%" align="center">
      <img
    src="https://readme-typing-svg.herokuapp.com?font=Fira+Code&weight=600&size=26&duration=3000&pause=1000&center=true&vCenter=true&width=900&color=00F0FF&lines=Hi+There!+👋;I'm+Mussarat+Shamsher;Agentic+AI+Engineer;AI+Automation+Developer;Full+Stack+Developer;Building+AI+Employees+%26+Intelligent+Systems"
    alt="Typing animation"
  />
  <p align="center">
  <img src="https://raw.githubusercontent.com/mussaratshamsher/mussaratshamsher/main/assets/hero-neon.gif"
       alt="Mussarat Shamsher — Agentic AI Engineer & Full Stack Developer"
       width="100%" />
</p>
    </td>
  </tr>
</table>


<p align="center">
  <img src="https://readme-typing-svg.herokuapp.com?font=Fira+Code&weight=600&size=26&duration=3000&pause=1000&center=true&vCenter=true&width=900&color=00F0FF&lines=Hi+There!+👋;I'm+Mussarat+Shamsher;Agentic+AI+Engineer;AI+Automation+Developer;Full+Stack+Developer;Building+AI+Employees+%26+Intelligent+Systems" />
</p>

<p align="center">
  <a href="https://mussarat-web-dev.vercel.app/">
    <img src="https://img.shields.io/badge/Portfolio-0D1117?style=for-the-badge&logo=vercel&logoColor=white" />
  </a>
  <a href="https://www.linkedin.com/in/mussarat-shamsher-7618a6380/">
    <img src="https://img.shields.io/badge/LinkedIn-0D1117?style=for-the-badge&logo=linkedin&logoColor=0A66C2" />
  </a>
  <a href="https://github.com/mussaratshamsher">
    <img src="https://img.shields.io/badge/GitHub-0D1117?style=for-the-badge&logo=github&logoColor=white" />
  </a>
</p>

---

# 👋 Hello, I'm Mussarat Shamsher

### 🤖 Agentic AI Engineer | Full Stack Developer

I build **AI agents, intelligent automation systems, digital employees, and full-stack AI applications** that solve real-world business problems.

🎓 **M.Sc. Economics** | **Certified AI Developer & Engineer (GIAIC)**

My work combines **Agentic AI + Backend Engineering + Modern Web Development** to create applications that can reason, use tools, interact with data, and automate workflows.

🎨 I also bring experience in **UI/UX, Graphic Design, Branding, and SEO**, allowing me to take products from concept to polished, deployable applications.

---

# 🧠 My Engineering Focus

```text
🤖 Agentic AI & AI Agents
⚡ AI Automation & Digital Employees
🧠 Multi-Agent Systems
🔗 MCP & Tool Calling
📚 RAG & Knowledge Systems
🌐 AI-Powered SaaS Applications
⚙️ Python & FastAPI Backend Systems
💻 Next.js & TypeScript Applications
🚀 Production AI Products
```

---

# 🚀 Featured Agentic AI Projects

## 🥇 LeadForge

<p align="center">
  <img src="https://raw.githubusercontent.com/mussaratshamsher/Capstone2-Backend_AI-Eng-Embedable_Widget/main/assets/leadforge-preview.png"
       alt="LeadForge — AI Lead Generation Platform"
       width="95%" />
</p>

### AI Lead Generation & Qualification Platform

AI-powered embeddable widget that **engages visitors, captures leads, and automates qualification**.

**Tech:** `Next.js` `TypeScript` `Python` `FastAPI` `AI Agents` `Supabase`

<p align="center">
  <a href="https://lead-forge-peach.vercel.app/">
    <img src="https://img.shields.io/badge/🚀%20Live%20Demo-00F0FF?style=for-the-badge&labelColor=0D1117" />
  </a>
  <a href="https://github.com/mussaratshamsher/Capstone2-Backend_AI-Eng-Embedable_Widget">
    <img src="https://img.shields.io/badge/💻%20GitHub-FFFFFF?style=for-the-badge&labelColor=0D1117" />
  </a>
</p>

---

## 🥈 Digital FTE

<p align="center">
  <img src="https://raw.githubusercontent.com/mussaratshamsher/digital-FTE/main/assets/digital-fte-preview.png"
       alt="Digital FTE — AI Workforce"
       width="95%" />
</p>

### Multi-Agent AI Workforce

AI-powered **Digital Employee platform** designed to automate business workflows through specialized AI agents.

**Tech:** `Next.js` `FastAPI` `OpenAI Agents SDK` `PostgreSQL` `Docker` `Kubernetes`

<p align="center">
  <a href="https://mussarat-digital-fte.vercel.app/">
    <img src="https://img.shields.io/badge/🚀%20Live%20Demo-00F0FF?style=for-the-badge&labelColor=0D1117" />
  </a>
  <a href="https://github.com/mussaratshamsher/digital-FTE">
    <img src="https://img.shields.io/badge/💻%20GitHub-FFFFFF?style=for-the-badge&labelColor=0D1117" />
  </a>
</p>

---

## 🥉 AI Content Intelligence

<p align="center">
  <img src="https://raw.githubusercontent.com/mussaratshamsher/Afynix-Digital/main/Week4-Content_Analyzer/assets/content-analyzer-preview.png"
       alt="AI Content Intelligence Platform"
       width="95%" />
</p>

### AI Content Analysis & Optimization

AI-powered platform that analyzes content and generates **actionable optimization insights**.

**Tech:** `Next.js` `TypeScript` `Python` `FastAPI` `AI`

<p align="center">
  <a href="https://mscontentpulse-sigma.vercel.app/">
    <img src="https://img.shields.io/badge/🚀%20Live%20Demo-00F0FF?style=for-the-badge&labelColor=0D1117" />
  </a>
  <a href="https://github.com/mussaratshamsher/Afynix-Digital/tree/main/Week4-Content_Analyzer">
    <img src="https://img.shields.io/badge/💻%20GitHub-FFFFFF?style=for-the-badge&labelColor=0D1117" />
  </a>
</p>

---

## 📚 Physical AI & Humanoid Robotics

<p align="center">
  <img src="https://raw.githubusercontent.com/mussaratshamsher/Nexe-Agentic-AI-Intern/master/02-Intermediate-level/assets/physical-ai-preview.png"
       alt="Physical AI & Humanoid Robotics"
       width="95%" />
</p>

### Interactive AI Learning Platform

AI-powered learning experience combining **technical knowledge with conversational AI**.

**Tech:** `Python` `Streamlit` `OpenAI` `Agentic AI`

<p align="center">
  <a href="https://humanoid-robotics-book-sepia.vercel.app/">
    <img src="https://img.shields.io/badge/🚀%20Live%20Demo-00F0FF?style=for-the-badge&labelColor=0D1117" />
  </a>
  <a href="https://github.com/mussaratshamsher/Nexe-Agentic-AI-Intern/tree/master/02-Intermediate-level">
    <img src="https://img.shields.io/badge/💻%20GitHub-FFFFFF?style=for-the-badge&labelColor=0D1117" />
  </a>
</p>

---

# 🧩 AI Engineering Stack

<p align="center">

<img src="https://skillicons.dev/icons?i=python,fastapi,nextjs,react,ts,postgres,supabase,docker,kubernetes,githubactions" />

</p>

### 🤖 Agentic AI

<p align="center">

<img src="https://img.shields.io/badge/OpenAI%20Agents%20SDK-412991?style=for-the-badge&logo=openai&logoColor=white" />
<img src="https://img.shields.io/badge/Gemini%20API-4285F4?style=for-the-badge&logo=google&logoColor=white" />
<img src="https://img.shields.io/badge/MCP-FF6B35?style=for-the-badge" />
<img src="https://img.shields.io/badge/RAG-6C63FF?style=for-the-badge" />

</p>

<p align="center">

<img src="https://img.shields.io/badge/Multi--Agent%20Systems-0D1117?style=for-the-badge" />
<img src="https://img.shields.io/badge/Tool%20Calling-0D1117?style=for-the-badge" />
<img src="https://img.shields.io/badge/AI%20Automation-0D1117?style=for-the-badge" />
<img src="https://img.shields.io/badge/Prompt%20Engineering-0D1117?style=for-the-badge" />

</p>

---

# 💻 Full Stack Development

| Area         | Technologies                                                         |
| ------------ | -------------------------------------------------------------------- |
| **Frontend** | Next.js • React • TypeScript • JavaScript • Tailwind CSS • ShadCN UI |
| **Backend**  | Python • FastAPI • REST APIs • Streamlit • Chainlit                  |
| **Database** | PostgreSQL • MySQL • SQLite • Supabase • Firebase                    |
| **Cloud**    | Vercel • Railway • Netlify • Hugging Face                            |
| **DevOps**   | Docker • Kubernetes • GitHub Actions • CI/CD                         |
| **Tools**    | Git • GitHub • Postman • Swagger                                     |

---

# 🎨 Beyond Development

I combine engineering with creative and growth skills:

**🎨 Design**
Figma • Canva • Photoshop • Illustrator • GIMP

**📈 SEO**
Technical SEO • Content Strategy • Website Optimization

This allows me to approach products from **engineering, design, usability, and growth** perspectives.

---

# 💼 Selected Full-Stack Projects

### 🛋️ Furniro

Modern e-commerce platform with authentication, payments, and responsive UI.

`Next.js` `TypeScript` `Tailwind CSS` `Clerk` `Stripe`

🔗 [Live Demo](https://functional-hackthon--two.vercel.app/)

### 🍽️ Flavour Fusion

Modern restaurant platform with blogs, animations, and responsive design.

`Next.js` `TypeScript` `Tailwind CSS`

🔗 [Live Demo](https://resturant-flavourfusion.vercel.app/)

### 🌐 Personal Portfolio

Personal website showcasing my AI engineering, full-stack, design, and SEO work.

`Next.js` `TypeScript` `Tailwind CSS` `Framer Motion`

🔗 [Visit Portfolio](https://mussarat-web-dev.vercel.app/)

---

# 📊 GitHub Analytics

<p align="center">
  <img width="49%" src="https://github-readme-stats-sigma-nine-80.vercel.app/api?username=mussaratshamsher&show_icons=true&theme=transparent&hide_border=true&rank_icon=github"/>

  <img width="49%" src="https://github-readme-streak-stats-eight.vercel.app/?user=mussaratshamsher&theme=transparent&hide_border=true"/>
</p>

<p align="center">
  <img width="98%" src="https://github-readme-activity-graph.vercel.app/graph?username=mussaratshamsher&theme=github-compact&hide_border=true"/>
</p>

---

# 🎯 Currently Building & Learning

<p align="center">

🤖 Agentic AI Engineering   •  
⚡ AI Employees & Digital Workforce

🧠 Multi-Agent Architecture   •  
🔗 MCP & Tool Calling

📚 RAG & Knowledge Systems   •  
🌐 AI SaaS Products

🚀 Production AI Applications

</p>

---

# 🌐 Let's Connect

<p align="center">

<a href="https://mussarat-web-dev.vercel.app/">
<img src="https://img.shields.io/badge/🌐%20Portfolio-0D1117?style=for-the-badge"/>
</a>

<a href="https://www.linkedin.com/in/mussarat-shamsher-7618a6380/">
<img src="https://img.shields.io/badge/LinkedIn-0D1117?style=for-the-badge&logo=linkedin&logoColor=0A66C2"/>
</a>

<a href="https://x.com/MussaratShams">
<img src="https://img.shields.io/badge/X-0D1117?style=for-the-badge&logo=x&logoColor=white"/>
</a>

<a href="https://github.com/mussaratshamsher">
<img src="https://img.shields.io/badge/GitHub-0D1117?style=for-the-badge&logo=github&logoColor=white"/>
</a>

</p>

---

<p align="center">

## 🚀 Building Intelligent Systems That Reason, Act & Automate.

### **Agentic AI Engineer • AI Automation • Full-Stack Development**

</p>

<p align="center">
  <i>“Build systems that don't just respond — they reason, act, and deliver.”</i>
</p>
